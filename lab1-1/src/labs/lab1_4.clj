...
(defn expand-word [word alphabet]
  (letfn [
          (filter-first [symb] (not= (first word) symb))
          (add-map [filtered-alphabet] (cons filtered-alphabet word))]
    (map add-map (filter filter-first alphabet))))          ; На выходе получим коллекцию
